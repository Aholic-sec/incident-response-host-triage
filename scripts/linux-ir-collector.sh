...
run_cmd() {
  ...
  printf '%s\t%s\t%s\n' "$rel" "$*" "$rc" >> "$OUTDIR/command-index.tsv"
...
run_shell() {
  ...
  dest="$OUTDIR/$rel"
  printf '[%s] $ %s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$desc" > "$dest"
  ...
  rc=$?
  printf '%s\t%s\t%s\n' "$rel" "$desc" "$rc" >> "$OUTDIR/command-index.tsv"
